...
class Compiler():
    def __init__(self,name,lang,task_id,commit_id):
        ...
        self.tests_dictionary = {     8:"C:\\Users\\akutalev\\Documents\\Visual Studio 2015\\Projects\\PythonApplication2\\PythonApplication2\\test1",
                                      1:"C:\\Users\\akutalev\\Documents\\Visual Studio 2015\\Projects\\PythonApplication2\\PythonApplication2\\test2", 
    ...
    def testing(self):
        ...
        for tests_number in range(len(input_files)):
            ...
            cmd_string =  " -m 1000 -t 1 -r " + self.exe_name + " -n no_config.txt" #TODO: сделать считывание из файла конфига
...
TestCode = Compiler("main.c",3,7,100500)
...
#    post_req = requests.post("http://10.4.0.113:5002/push_result",json = {"commit_id":TestCode.commit_id,"result_code":rezult,"output":TestCode.verdict})
#else:
 #   post_req = requests.post("http://10.4.0.113:5002/push_result",json = {"commit_id":TestCode.commit_id,"result_code":compile_rezult,"output":TestCode.verdict})
 #   print ("qwerty")
...
#compile_rezult= TestCode.compile():wq
#print(compile_rezult)
#if compile_rezult == "OK":
#    x = TestCode.testing()

#response = requests.get("http://10.4.0.113:1914/check",TestCode.compilers_dictionary.keys())
#print ()

#def

while (1):
   print (TestCode.compilers_dictionary.keys(),"\n")
   sub = requests.get("http://10.4.0.113:5004/get_not_tested_submit")
   submit = json.loads(sub.text)
   if submit != []:
        submit = json.loads(submit) 
        if submit['compiler_id']!=3:
            print("mdee\n")
            time.sleep(5)
        else:
            ...
            if compile_rezult == "OK":
                ...
                print(TestCode.verdict)
                print('\n\n\n\n')
                post_req = requests.post("http://10.4.0.113:5004/push_result",json = {"commit_id":TestCode.commit_id,"result_code":rezult,"output":TestCode.verdict})
            else:
                post_req = requests.post("http://10.4.0.113:5004/push_result",json = {"commit_id":TestCode.commit_id,"result_code":compile_rezult,"output":TestCode.verdict})
                print(compile_rezult, TestCode.verdict)
   else:
        time.sleep(5)
